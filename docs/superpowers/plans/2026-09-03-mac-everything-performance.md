...
| 普通关键词搜索延迟 | < 10ms | **9 ms** ✅ |
| 正则搜索延迟 | < 30ms | **20 ms** ✅ |
| 索引文件大小 | < 50MB | **55.3 MB** ⚠️（去重后；见下方说明） |
| 冷启动 | < 500ms | **19 ms** ✅ |
| 搜索期间主线程阻塞 | 0（后台执行） | 已后台化 ✅ |
| 待机 CPU | ≈0 | 待修（500ms 轮询 + App Nap 禁用） |

> 任何导致以上指标退化的改动，按 AGENTS.md「性能不达标停机」立即回退并先解决退化。

**索引大小缺口说明**：当前 55.3MB 由 40MB 记录数组（40 字节/条）+ ~15MB 名称池（已做小写去重）构成。要压到 <50MB 需再缩记录体：`size`/`modified_time` 由 u64 降为 u32（但 `size` 上限变 4GB，对超大文件是正确性风险），或移除 `name_lower` 改为查询时大小写不敏感匹配（牺牲搜索速度，与「性能第一」冲突）。此为速度/体积取舍，按 AGENTS.md 停机规则**待用户确认**后实施。

---

## 5. 下一步

D1/D2/D3 已确认。接下来将 Phase 1–5 逐条展开为 bite-sized TDD 任务（含完整代码与验收命令，遵循 writing-plans 的粒度标准），并优先落地 **Phase 0（基准测试闸门）→ Phase 1（搜索链路）**。
