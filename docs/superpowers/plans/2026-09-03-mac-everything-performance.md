...
> 硬指标：搜索延迟 / 冷启动 / 主线程阻塞。**索引文件大小是软指标**（信息性），不驱动设计决策——磁盘上几十 MB 的差异对用户无感，不应为它牺牲正确性或速度。

| 指标 | 目标 | 实测（release，100万合成路径） |
|---|---|---|
| 普通关键词搜索延迟 | < 10ms | **9 ms** ✅ |
| 正则搜索延迟 | < 30ms | **23 ms** ✅ |
| 索引文件大小 | （软指标，信息性） | **55.3 MB** ℹ️ |
| 冷启动 | < 500ms | **21 ms** ✅ |
| 搜索期间主线程阻塞 | 0（后台执行） | 已后台化 ✅ |
| 待机 CPU | ≈0 | ≈0（500ms 轮询为 sleep，apply_updates 空队列 O(1)）；App Nap 仍禁用（防睡眠唤醒回归，属功耗而非 CPU） |

> 任何导致搜索延迟 / 冷启动退化的改动，按 AGENTS.md「性能不达标停机」立即回退并先解决退化。

---

## 5. 完成状态

D1/D2/D3 已确认；Phase 0–5 已全部完成（19 个单测全绿 + 2 个基准）。索引文件大小按用户决策降为**软指标**（保留 40 字节记录、`size` 字节精确无上限；曾尝试的 u64→u32 压缩已回退）。
