...
impl QueryParser {
    pub fn parse(query: &str) -> QueryNode {
        ...
        if query_trim.starts_with("regex:") {
            ...
            return QueryNode::Contains("".to_string());
        }
    ...
    fn parse_size_op(term: &str) -> Option<QueryNode> {
        let is_gt = term.starts_with('>');
        let is_lt = term.starts_with('<');
        if !is_gt && !is_lt { return None; }
        
        let num_part = &term[1..];
        let mut multiplier = 1u64;
        let mut num_str = num_part;
        
        if num_part.ends_with("kb") { multiplier = 1024; num_str = &num_part[..num_part.len()-2]; }
        else if num_part.ends_with("mb") { multiplier = 1024*1024; num_str = &num_part[..num_part.len()-2]; }
        else if num_part.ends_with("gb") { multiplier = 1024*1024*1024; num_str = &num_part[..num_part.len()-2]; }
        
        if let Ok(val) = num_str.parse::<f64>() {
            let bytes = (val * multiplier as f64) as u64;
            if is_gt { return Some(QueryNode::Size(SizeOp::Gt(bytes))); }
            if is_lt { return Some(QueryNode::Size(SizeOp::Lt(bytes))); }
        }
        None
    }

    fn parse_date_op(term: &str) -> Option<QueryNode> {
        if term == "today" { return Some(QueryNode::Date(DateOp::Today)); }
        if term == "yesterday" { return Some(QueryNode::Date(DateOp::Yesterday)); }
        
        // Basic <yyyy-mm-dd or >yyyy-mm-dd parser
        // (For simplicity, not implementing full date parsing in Rust here, just returning dummy for now
        // if needed, or rely on swift side, but we need it here if evaluating in rust.)
        None // Implement full date parse later if needed
    }
}
